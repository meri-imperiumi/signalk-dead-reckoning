/**
 * Polar speed model from the Signal K `polars` resource.
 *
 * Consumes the resource contract published by polar tools (e.g.
 * signalk-polar-management): the `polars.activePolar` delta points at
 * the selected polar resource, and `polars.performanceFactor` carries a
 * crew-set derating multiplier. The canonical table is a TWS × TWA grid
 * in SI units (m/s, rad) with a port/starboard symmetry flag:
 *
 *   { kind: 'polarTable', axes: { tws: [...], twa: [...] },
 *     values: { boatSpeedMatrix: [twsIdx][twaIdx] },
 *     symmetry: { portStarboardSymmetric: true } }
 *
 * This mirrors signalk-energy-predictor's polar module (same contract,
 * same interpolation semantics) so both plugins agree on what an
 * installed polar says. No provider, no active polar, or a malformed
 * table all degrade to null — the resource path is strictly optional
 * and needs no configuration beyond selecting an active polar.
 *
 * Pure logic plus one in-process resource fetch — unit-testable with a
 * stub `app.resourcesApi`.
 *
 * @file resource-polar.js
 */
 ...
 *
 * Accepts the plain delta form (`{href: "/resources/polars/<id>"}`),
 * the `app.getSelfPath`-wrapped form (`{value: {href: ...}}`), and full
 * API URLs (`/signalk/v1/api/resources/polars/<id>`).
 *
 ...
 * - TWS above the maximum: clamps to the last row — conservative for a
 *   DR fallback (a boat at hull speed does not get faster with more
 *   wind; overestimating speed there would over-run the DR position).
 *
 ...
 *
 * @param {object} params
 * @param {string} params.id - Resource id (for logging/status)
 ...
 *
 * Reads the `polars.activePolar` pointer via `readValue` (deltaState
 * first, `getSelfPath` fallback), fetches the referenced resource
 * in-process, and applies the published `polars.performanceFactor`.
 * Returns null — never throws — when no polar is active, no provider
 * is installed, or the fetch/shape fails: the caller stays on its
 * other speed sources.
 *
 * @param {object} params
